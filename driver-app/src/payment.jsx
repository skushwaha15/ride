...
function Payment({ isOpen, onClose, amount, rideDetails, onPaymentComplete, userType = 'user', cardOnly = false }) {
  ...
  const handlePayment = async () => {
    ...
    try {
      if (['CASH', 'UPI', 'WALLET'].includes(selectedMethod)) {
          ...
          <>
            ...
            {selectedMethod === 'CARD' ? (
              stripePromise ? (
                ...
                <div style={styles.errorContainer}>
                  <span>Stripe publishable key is missing. Add REACT_APP_STRIPE_PUBLISHABLE_KEY in driver-app/.env.</span>
                </div>
              )
            ) : selectedMethod && (
              <button
              ...
              >
                {processing ? 'Processing...' : userType === 'driver' ? `Collect ₹${amount}` : `Pay ₹${amount}`}
              </button>
            )}
          </>
        )}
